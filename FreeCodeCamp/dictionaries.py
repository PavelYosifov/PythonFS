...
dog = {
    "name": "Roger",
    "age": 8,
}  # dictionary with keys name and age and values Roger and 8
...
print(
    dog.get("color", "brown")
)  # prints the color using the key name with get function which was not present in the dictionary but .get created it.
print(
    dog.pop("name")
)  # prints the name using the key name with pop function and removes it from the dictionary
print(
    dog.popitem()
)  # prints the last key and value with popitem function and removes it from the dictionary
print(
    "color" in dog
)  # checks if key color is present in the dictionary, will return True or False
